...
import {
  createCalculatedInsightTool,
  createCalculatedInsightInputSchema
} from "./tools/ci/create-calculated-insight.js";
...
import {
  ...
  getCalculatedInsightStatusInputSchema
} from "./tools/ci/get-calculated-insight-status.js";
import {
...
// Identity Resolution
import {
  listIdentityResolutionsTool,
  listIdentityResolutionsInputSchema
} from "./tools/identity/list-identity-resolutions.js";
import {
  describeIdentityResolutionTool,
  describeIdentityResolutionInputSchema
} from "./tools/identity/describe-identity-resolution.js";
...
import {
  listActivationTargetsTool,
  listActivationTargetsInputSchema
} from "./tools/activation/list-activation-targets.js";
...
server.tool(
  "list_dmos",
  "List all Data Model Objects",
  listDmosInputSchema.shape,
  safeTool((p) => listDmosTool(p, auth, http))
);

server.tool(
  "describe_dmo",
  "Describe a DMO with its fields",
  describeDmoInputSchema.shape,
...
server.tool(
  ...
  createDmoInputSchema.shape,
  safeTool((p) => createDmoTool(p, auth, http))
);

server.tool(
  "create_dmo_from_dlo",
  "Create DMO from DLO with auto type correction and mapping",
  createDmoFromDloInputSchema.shape,
...
server.tool(
  ...
  deleteDmoInputSchema.shape,
  safeTool((p) => deleteDmoTool(p, auth, http))
);

server.tool(
  "list_dmo_mappings",
  "List field mappings between DLO and DMO",
  listDmoMappingsInputSchema.shape,
  safeTool((p) => listDmoMappingsTool(p, auth, http))
);

server.tool(
  "create_dmo_mapping",
  "Create field mapping between DLO and DMO",
  createDmoMappingInputSchema.shape,
...
server.tool(
  ...
  listCalculatedInsightsInputSchema.shape,
  safeTool((p) => listCalculatedInsightsTool(p, auth, http))
);

server.tool(
  "create_calculated_insight",
  "Create a Calculated Insight with smart schedule translation",
  createCalculatedInsightInputSchema.shape,
...
server.tool(
  ...
  runCalculatedInsightInputSchema.shape,
  safeTool((p) => runCalculatedInsightTool(p, auth, http))
);

server.tool(
  "get_calculated_insight_status",
  "Get status of a Calculated Insight",
  getCalculatedInsightStatusInputSchema.shape,
  safeTool((p) => getCalculatedInsightStatusTool(p, auth, http))
);

server.tool(
  "delete_calculated_insight",
...
server.tool(
  ...
  querySqlInputSchema.shape,
  safeTool((p) => querySqlTool(p, auth, http))
);

server.tool(
  "describe_table",
  "Describe table columns via LIMIT 0 query",
  describeTableInputSchema.shape,
  safeTool((p) => describeTableTool(p, auth, http))
);

server.tool(
  "search_vector",
  "Vector similarity search on a search index",
  searchVectorInputSchema.shape,
  safeTool((p) => searchVectorTool(p, auth, http))
);

server.tool(
  "search_hybrid",
  "Hybrid keyword+vector search on a search index",
  searchHybridInputSchema.shape,
...
server.tool(
  ...
  listDataStreamsInputSchema.shape,
...
server.tool(
  ...
  describeDataStreamInputSchema.shape,
...
server.tool(
  ...
  createDataStreamInputSchema.shape,
...
server.tool(
  ...
  listTransformsInputSchema.shape,
...
server.tool(
  ...
  runTransformInputSchema.shape,
  safeTool((p) => runTransformTool(p, auth, http))
);

server.tool(
  "get_transform_status",
  "Get status of a data transform",
  getTransformStatusInputSchema.shape,
  safeTool((p) => getTransformStatusTool(p, auth, http))
);

// Identity Resolution tools
server.tool(
  "list_identity_resolutions",
  "List all identity resolution rulesets",
  listIdentityResolutionsInputSchema.shape,
  safeTool((p) => listIdentityResolutionsTool(p, auth, http))
);

server.tool(
  "describe_identity_resolution",
  "Describe an identity resolution ruleset",
  describeIdentityResolutionInputSchema.shape,
...
server.tool(
  "describe_segment",
  "Describe a segment",
  describeSegmentInputSchema.shape,
...
server.tool(
  "publish_segment",
  "Publish a segment",
  publishSegmentInputSchema.shape,
...
server.tool(
  ...
  listActivationsInputSchema.shape,
  safeTool((p) => listActivationsTool(p, auth, http))
);

server.tool(
  "list_activation_targets",
  "List all activation targets",
  listActivationTargetsInputSchema.shape,
...
server.tool(
  ...
  createActivationInputSchema.shape,
...
server.tool(
  ...
  listDataActionsInputSchema.shape,
...
server.tool(
  "query_profile",
  "Query unified profiles using Data Cloud token",
  queryProfileInputSchema.shape,
  safeTool((p) => queryProfileTool(p, auth, http))
);

// Credit tools
server.tool(
  "estimate_flex_credits",
  "Estimate or query live flex credit usage",
  estimateFlexCreditsInputSchema.shape,
...
server.tool(
  "resolve_field_names",
  "Resolve CRM object/field names to DLO/DMO names",
  resolveFieldNamesInputSchema.shape,
